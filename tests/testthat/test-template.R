...
#    `functionName` is a function's name in your module (e.g., `fireWeather`).
# 2. Copy this file to the tests folder (i.e., `C:/Ceres/GitHub/LandscapesInMotion/R/SpaDES/m/fireWeather/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("fireWeather")
  ...
  parameters <- list(
    ...
    fireWeather = list(.saveInitialTime = NA)
  )
  ...
  if (exists("fireWeatherEvent1", envir = .GlobalEnv)) {
    simOutput <- fireWeatherEvent1(mySim)
  } else {
  ...
  if (exists("fireWeatherEvent2", envir = .GlobalEnv)) {
    simOutput <- fireWeatherEvent2(mySim)
  } else {
